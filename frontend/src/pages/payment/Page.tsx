          ...
          <div className="mb-8">
            ...
            <div className="max-w-2xl mx-auto mb-8">
              <div className="flex items-center justify-between relative">
                ...
                <div
                  className="absolute top-1/2 left-0 h-0.5 bg-gradient-to-r from-white to-white/80 -translate-y-1/2 z-0 transition-all duration-500"
              ...
              {currentStep === 1 && !showBnplAmountSelector && (
                <div className="space-y-4">
                  <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              현재 잔액:{" "}
                              <span className="font-medium">
                                {remainAccount.toLocaleString()}원
                              </span>
                            </p>
                            {/* 잔액 부족 힌트 */}
                            {remainAccount < paymentInfo.finalPrice && (
                              <p className="text-xs text-amber-600 mt-1">
                                {(
                                  paymentInfo.finalPrice - remainAccount
                                ).toLocaleString()}
                                원 부족
                              </p>
                            )}
                    ...
                    {/* BNPL 결제 */}
                    {(() => {
                      const bnplDisabled = bnplBalance <= 0;

                      return (
                        <div
                          role="button"
                          tabIndex={bnplDisabled ? -1 : 0}
                          aria-disabled={bnplDisabled}
                          onClick={() => {
                            if (bnplDisabled) return; // 🔒 잔액 0원이면 클릭 무시
                        ...
                        >
                          ...
                          {/* BNPL 잔액 부족 안내 (잔액이 0은 아니고, 분할 필요한 경우만) */}
                          {!bnplDisabled && needsSplitPayment && (
                            <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                              ...
                              <div className="text-xs text-orange-700 mt-1 ml-6">
                                BNPL 잔액: {bnplBalance.toLocaleString()}원 / 내
                                계좌: {cashAmount.toLocaleString()}원 결제
                                {remainAccount < cashAmount && (
                                  <span className="ml-2 text-red-600 font-medium">
                                    (일반 계좌 잔액 부족{" "}
                                    {Math.max(
                                      0,
                                      cashAmount - remainAccount
                                    ).toLocaleString()}
                                    원)
                                  </span>
                                )}
                ...
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="font-semibold">
                        {immediateToUse > 0
                          ? immediateToUse.toLocaleString() + "원"
                          : "0원"}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span
                        className={`font-semibold ${
                          afterBalance < 0 ? "text-red-600" : "text-slate-900"
                        }`}
                      >
                        {(afterBalance >= 0
                          ? afterBalance
                    ...
                    {insufficient && (
                      <div className="mt-3 text-xs text-red-700">
                        일반 계좌 잔액이 부족합니다. 부족 금액:{" "}
                        {(immediateToUse - remainAccount).toLocaleString()}원
                      </div>
                    )}
                  </div>
                  ...
                  <Button
                    ...
                    title={
                      insufficient ? "잔액 부족으로 결제할 수 없습니다." : ""
                    }
                  ...
                  {insufficient && (
                    <p className="mt-2 text-center text-sm text-red-600">
                      일반 계좌 잔액이 부족하여 결제할 수 없습니다. 부족 금액:{" "}
                      {(immediateToUse - remainAccount).toLocaleString()}원
                    </p>
                  )}
