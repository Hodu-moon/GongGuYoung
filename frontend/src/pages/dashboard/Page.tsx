...
function RankEmblem({ rank }: { rank: number }) {
  const styles = [
    { bg: "from-yellow-300 to-amber-500", ring: "ring-amber-300", icon: "text-amber-800" },
    { bg: "from-gray-200 to-gray-400", ring: "ring-gray-300", icon: "text-gray-700" },
    { bg: "from-orange-300 to-amber-600", ring: "ring-orange-300", icon: "text-amber-900" },
  ];
  const s = styles[Math.min(rank - 1, 2)];
  return (
    <div className={`relative inline-flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br ${s.bg} ring-2 ${s.ring} shadow-md`} aria-label={`${rank}위`} title={`${rank}위`}>
...
export default function DashboardPage() {
  ...
  type IndexRow = {
    ...
    prodI: string;
    endTs: number;
    derivedStatus: "active" | "completed" | "cancelled";
  };

  const index: IndexRow[] = useMemo(() => {
    const now = Date.now();
    return legacy.map((c) => {
      ...
      return {
        c,
        titleL,
        titleI: toInitials(titleL),
        prodL,
        prodI: toInitials(prodL),
        endTs,
  ...
  const stats = useMemo(() => {
    ...
    const completedCount = legacy.filter((c) => new Date(c.endDate).getTime() <= now).length;
    const totalParticipants = legacy.reduce((sum, c) => sum + c.participants, 0);
    const totalFunding = legacy.reduce((sum, c) => sum + c.discountPrice * c.participants, 0);
    return { activeCampaigns: activeCount, totalParticipants, completedCampaigns: completedCount, totalFunding };
  }, [legacy]);
  ...
  const trendingCampaigns = useMemo(
    () =>
      legacy
        .filter((c) => c.status === "active" && new Date(c.endDate).getTime() > Date.now())
          ...
          <div className="container mx-auto px-4 py-3">
            <div className="flex justify-between items-center">
              <Link to="/dashboard" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                <Image src="/favicon.png" alt="GongGuYoung Smart Campus" width={36} height={36} className="rounded-lg" />
                <div>
                    ...
                    <CardContent className="space-y-4">
                      {trendingCampaigns.map((campaign, index) => (
                        <div key={campaign.id} className="flex items-center gap-4 p-4 bg-purple-50/50 rounded-lg hover:bg-purple-100/50 transition-colors">
                          ...
                          <img src={campaign.product.imageUrl || "/placeholder.svg"} alt={campaign.product.name} className="w-12 h-12 object-cover rounded-lg" />
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-purple-900 truncate">{campaign.title}</h4>
                            <p className="text-sm text-purple-600 flex items-center gap-2 mt-1">
                              <Users className="h-4 w-4" />{campaign.participants}명 참여
                              <Eye className="h-3 w-3" />{campaign.viewCount}
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="font-bold text-purple-700">
                              {(campaign.discountPrice * (100 - getDiscountRate(campaign.targetQuantity)) * 0.01).toLocaleString()}원
                            </p>
                      ...
                      {recentCampaigns.map((campaign) => (
                        <Link key={campaign.id} to={`/campaigns/${campaign.id}`} className="block">
                          <div className="p-4 border border-purple-200/50 bg-purple-50/30 rounded-lg hover:border-purple-300 hover:bg-purple-100/40 hover:shadow-md transition-all duration-200">
                            <div className="flex gap-3">
                              <img src={campaign.product.imageUrl || "/placeholder.svg"} alt={campaign.product.name} className="w-16 h-16 object-cover rounded-lg" />
                              <div className="flex-1 min-w-0">
                                <h4 className="font-bold text-purple-900 text-sm mb-2 line-clamp-2">{campaign.title}</h4>
                                <div className="text-xs text-purple-600 mb-2 flex items-center gap-2">
                                  <Eye className="h-3 w-3" />{campaign.viewCount}
                                  <Users className="h-3 w-3 ml-2" />{campaign.participants}
                                </div>
                                <div className="flex items-center justify-between">
